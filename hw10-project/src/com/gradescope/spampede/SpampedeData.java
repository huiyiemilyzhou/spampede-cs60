...
class SpampedeData {
	...
	// moves head forward by one
	public void updateHead(BoardCell cell){
	...
	protected BoardCell getNorthNeighbor(BoardCell cell) {
        ...
        return this.boardCells2D[(row - 1 + this.boardCells2D.length) % this.boardCells2D.length][col];
	}
	...
	protected BoardCell getSouthNeighbor(BoardCell cell) {
		int row = cell.getRow();
    	int col= cell.getColumn();
    	return this.boardCells2D[(row + 1) % this.boardCells2D.length][col];
	}
	...
	protected BoardCell getEastNeighbor(BoardCell cell) {
		int row = cell.getRow();
		int col = cell.getColumn();
		return this.boardCells2D[row][(col + 1) % this.boardCells2D[0].length];
	}
	...
	protected BoardCell getWestNeighbor(BoardCell cell) {
		int row = cell.getRow();
    	int col = cell.getColumn();
   		return this.boardCells2D[row][(col - 1 + this.boardCells2D[0].length) % this.boardCells2D[0].length];
	}
	...
	public BoardCell getNextCellInDir() {
		...
		} else if (currentMode == SnakeMode.GOING_SOUTH) {
			return getSouthNeighbor(head);
		} else if (currentMode == SnakeMode.GOING_WEST) {
			return getWestNeighbor(head);
		} else if (currentMode == SnakeMode.GOING_EAST) {
  ...
  public void reverseSnake() {
	...
	while(snakeCells.size() != 0){
		newsnakeCells.addFirst(snakeCells.getFirst());
	...
	} else if(this.getWestNeighbor() == neck){
		this.setDirectionEast();
