...
class SpampedeDisplay {
	...
	public void updateGraphics() {
		...
		int xchange = (this.width - (this.theData.getNumColumns() * Preferences.CELL_SIZE)) / 2;
		int ychange= (this.height - (this.theData.getNumRows() * Preferences.CELL_SIZE)) / 5;


		for (int i = 0; i < this.theData.getNumRows(); i++){
			for (int j = 0; j < this.theData.getNumColumns(); j++){

				int x = xchange + j * Preferences.CELL_SIZE;
				int y = ychange + i * Preferences.CELL_SIZE;


				Color cellColor = this.theData.getCellColor(i, j);
